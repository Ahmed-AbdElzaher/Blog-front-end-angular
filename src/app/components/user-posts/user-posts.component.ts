...
export class UserPostsComponent implements OnInit {
  ...
  constructor(
    ...
    private _isLogged: IsLoggedInService
  ) {}
  ...
  ngOnInit(): void {
    if (!this._isLogged.isLoggedIn()) {
      ...
      this._Http
        .get('http://localhost:5000/api/posts/userPosts', httpOptions)
        .subscribe(
          (response: any) => {
  ...
  delete(currentIndex: number): void {
    ...
    if (deletePost) {
      ...
      this._Http
        .delete(`http://localhost:5000/api/posts/${post._id}`, httpOptions)
        .subscribe(
          (response: any) => {
  ...
  update(currentIndex: number): void {
    ...
    if (editPost) {
      ...
      this._Http
        .put(`http://localhost:5000/api/posts/${post._id}`, post, httpOptions)
        .subscribe(
          (response: any) => {
            this.posts.splice(currentIndex);
          },
